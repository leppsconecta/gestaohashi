...
const FichaTecnicaPage: React.FC = () => {
  ...
  const FichaTecnicaForm = ({ initial, onSave, onCancel }: { initial?: PratoFicha, onSave: (p: PratoFicha) => void, onCancel: () => void }) => {
    ...
    const handleAddIngredient = () => {
      ...
      const newIng: IngredientePrato = { materiaPrimaId: selectedInsumoId, quantidade: Number(qtdToAdd) };
      setFormData(prev => ({
        ...prev,
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                  ...
                  {activeTab === 'categorias' && (
                    <div className="p-8 text-center text-slate-400">
                      <p>Gerenciamento simplificado de categorias.</p>
                      <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-4">
                        {categorias.map(c => (
                          <div key={c.id} className="p-4 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 font-bold text-slate-600 dark:text-slate-300">
                            {c.nome}
                          </div>
                        ))}
                      </div>
