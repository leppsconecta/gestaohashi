...
const FichaTecnicaPage: React.FC = () => {
      ...
      content: (
        <div className="space-y-4">
          <div className="space-y-1">
            ...
            <input id="cat-name" type="text" className="w-full px-4 py-2 border rounded-lg outline-none focus:ring-2 focus:ring-red-500/20" placeholder="Ex: Bebidas" />
          </div>
        </div>
      ),
      onConfirm: () => {
      ...
      content: (
        <div className="space-y-5">
          <div className="space-y-1">
            ...
            <input id="ins-name" type="text" defaultValue={initial?.nome} className="w-full px-4 py-2 border rounded-lg" placeholder="Ex: Arroz" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="text-xs font-bold text-slate-500 uppercase">Unidade</label>
              <select id="ins-unit" defaultValue={initial?.unidade || 'kg'} className="w-full px-4 py-2 border rounded-lg">
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-slate-500 uppercase">Custo Unitário</label>
              <div className="relative">
                <span className="absolute left-3 top-2 text-slate-400">R$</span>
                <input id="ins-cost" type="number" step="0.01" defaultValue={initial?.custoUnitario} className="w-full pl-10 pr-4 py-2 border rounded-lg" placeholder="0,00" />
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="space-y-6 max-w-full overflow-hidden">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 text-center">
            ...
            <p className="text-xl font-black text-slate-900">R$ {custoTotal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
          </div>
          <div className="bg-rose-50 p-4 rounded-xl border border-rose-100 text-center">
            ...
            <div className="flex items-center justify-center gap-1">
              ...
              <input 
                ...
                className="bg-transparent text-xl font-black text-rose-600 outline-none w-24 text-center" 
              />
            </div>
          </div>
          <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100 text-center">
            ...
            <p className="text-xl font-black text-emerald-600">{margem}%</p>
          </div>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="space-y-4">
             <div className="aspect-square bg-slate-50 border-2 border-dashed border-slate-200 rounded-3xl flex flex-col items-center justify-center text-slate-400 hover:text-indigo-600 transition-all cursor-pointer">
             ...
             <div className="space-y-3">
               <div className="space-y-1">
                 ...
                 <input id="dish-name" type="text" defaultValue={initial?.nome} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-100 rounded-xl focus:ring-4 focus:ring-red-500/10 outline-none" placeholder="Ex: Sashimi Misto" />
               </div>
               <div className="space-y-1">
                 <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Categoria</label>
                 <select id="dish-cat" defaultValue={initial?.categoriaId} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-100 rounded-xl outline-none">
          ...
          <div className="space-y-6">
            <div className="space-y-2">
               ...
               <div className="flex gap-2">
                 <select 
                    className="flex-1 px-4 py-2.5 border rounded-xl outline-none text-sm min-w-0"
                 ...
                 <input 
                    type="number" 
                    className="w-24 px-4 py-2.5 border rounded-xl outline-none text-sm" 
                    placeholder="Qtd"
                    value={qtd || ''}
               ...
               <div className="overflow-x-auto">
                 <table className="w-full text-xs min-w-[300px]">
                   <thead className="bg-slate-100">
                     <tr>
                       <th className="px-4 py-2 text-left font-bold text-slate-500">INGREDIENTE</th>
                       <th className="px-4 py-2 text-center font-bold text-slate-500">QTD</th>
                       <th className="px-4 py-2 text-right font-bold text-slate-500">CUSTO</th>
                      ...
                      {localIngredientes.length > 0 ? localIngredientes.map((ing, idx) => {
                        ...
                        return (
                          <tr key={idx} className="hover:bg-slate-100/50 transition-colors">
                            <td className="px-4 py-2 font-medium text-slate-700 truncate max-w-[100px]">{insumo?.nome}</td>
                            <td className="px-4 py-2 text-center font-bold whitespace-nowrap">{ing.quantidade} {insumo?.unidade}</td>
                            <td className="px-4 py-2 text-right font-bold text-red-600 whitespace-nowrap">R$ {custoIng.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</td>
        ...
        <div className="space-y-1">
          ...
          <textarea id="dish-preparo" defaultValue={initial?.modoPreparo} className="w-full h-32 px-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:ring-4 focus:ring-red-500/10 resize-none text-sm" placeholder="1. Descreva o passo a passo..."></textarea>
        </div>
      </div>
    );
  };
  ...
  const pratosColumns: Column<PratoFicha>[] = [
    ...
    { header: 'NOME', accessor: (p) => (
      <div className="flex flex-col min-w-0">
        <span className="font-bold text-slate-900 truncate">{p.nome}</span>
    ...
    { header: 'CUSTO', accessor: (p) => <span className="font-bold text-slate-500 whitespace-nowrap">R$ {p.custoTotal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>, className: 'w-28' },
    { header: 'PREÇO', accessor: (p) => <span className="font-black text-slate-900 text-lg whitespace-nowrap">{p.precoVenda.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>, className: 'w-28' },
    { header: 'MG %', accessor: (p) => <span className="font-black text-emerald-600">{Math.round(((p.precoVenda - p.custoTotal) / p.custoTotal) * 100)}</span>, className: 'w-20' },
  ...
  const insumosColumns: Column<MateriaPrima>[] = [
    { header: 'NOME', accessor: 'nome', className: 'font-bold text-slate-900' },
    { header: 'UNIDADE', accessor: 'unidade', className: 'text-slate-500 font-medium' },
    { header: 'CUSTO / UNID', accessor: (i) => <span className="font-bold text-slate-800">R$ {i.custoUnitario.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span> },
        ...
        <div className="flex border-b border-slate-100 dark:border-slate-800 overflow-x-auto no-scrollbar">
          {[
            ...
            <button
              ...
              className={`flex-1 min-w-[120px] flex items-center justify-center gap-2 py-4 text-xs font-bold uppercase tracking-widest transition-all border-b-2 ${
                activeTab === tab.id 
        ...
        <div className="p-4 sm:p-6 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
             <div className="relative w-full sm:w-80">
               ...
               <input type="text" className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-xl outline-none focus:ring-4 focus:ring-red-500/10 text-sm font-medium dark:text-white" placeholder={`Buscar ${activeTab}...`} />
             </div>
             {activeTab === 'pratos' && (
