                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex-1 space-y-0.5">
                        <span className="text-3xl font-black tabular-nums tracking-tighter masonry-tracking-tight leading-none block">
                          {card.value}
                        </span>
                        <p className={`text-[10px] font-semibold ${card.alert ? 'text-red-100' : 'text-slate-500 tracking-tight'}`}>
                          Pendentes
                        </p>
                      </div>
                      <div className={`w-px h-10 shrink-0 ${card.alert ? 'bg-white/30' : 'bg-slate-100 dark:bg-slate-800'}`} />
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 border border-slate-100 dark:border-slate-800 shadow-sm space-y-8">
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              <h3 className="text-xl font-bold text-slate-950 dark:text-white tracking-tight">Reservas semanais</h3>
              <p className="text-sm text-slate-600 dark:text-slate-400 font-medium">Fluxo de agendamentos por dia</p>
            </div>
            <div className="text-right">
              <span className="text-3xl font-black text-red-700 dark:text-red-400 tracking-tight">
                {weeklyTotal}
              </span>
              <p className="text-[10px] font-bold text-slate-500 tracking-wider">Total na semana</p>
            </div>
          </div>

          <div className="flex items-end justify-between gap-3 h-56 pt-8 pb-2">
            {weeklyData.map((d, i) => {
              const isToday = i === 4;
              return (
                <div key={i} className="flex-1 flex flex-col items-center gap-3 group h-full">
                  ...
                  <span className={`text-[11px] font-bold ${isToday ? 'text-red-700 dark:text-red-400' : 'text-slate-50'}`}>
                    {d.day}
                  </span>
                </div>
              );
            ...
            <div className="relative w-36 h-36 shrink-0">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-[10px] font-bold text-emerald-600">Positivo</p>
                </div>
              </div>
            </div>
