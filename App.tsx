    ...
    <div className="h-[calc(100vh-12rem)] flex flex-col items-center justify-center p-4 animate-in fade-in zoom-in-95 duration-500">
      <div className="w-full max-w-md bg-white dark:bg-slate-900 p-8 sm:p-10 rounded-[2.5rem] shadow-xl border border-slate-100 dark:border-slate-800 text-center space-y-6">
        ...
        <div className="space-y-1">
          ...
          <div className="mt-2 py-1.5 px-3 bg-slate-50 dark:bg-slate-800 rounded-lg border border-slate-100 dark:border-slate-700 inline-block">
            <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Senha de acesso:</p>
            <p className="text-sm font-black text-red-600 dark:text-red-400 tracking-[0.2em]">281084</p>
          </div>
        </div>
        ...
        <form onSubmit={handleUnlock} className="space-y-4">
          <div className="relative">
            <input 
              autoFocus
              type="password"
              placeholder="Digite 281084"
              value={password}
              onChange={(e) => setPassword(e.target.value)}
              disabled={loading}
              className={`w-full px-6 py-4 bg-slate-50 dark:bg-slate-950 border rounded-2xl outline-none transition-all text-center text-xl font-black tracking-[0.4em] focus:ring-4 placeholder:text-slate-300 placeholder:tracking-normal placeholder:font-bold ${
                error 
